...
- **Multi-PG**: native builds for **PostgreSQL 16, 17, and 18**, each
  compiled against the matching `postgresql-server-dev-*` so
  `PG_MODULE_MAGIC` lines up with the target server.
- **Multi-arch**: first-class support for both **AMD64 / x86_64** and
...
| PostgreSQL | Package | AMD64 | ARM64 |
| --- | --- | :---: | :---: |
| 16 | `postgresql-16-fractalsql-<arch>` | ✓ | ✓ |
| 17 | `postgresql-17-fractalsql-<arch>` | ✓ | ✓ |
| 18 | `postgresql-18-fractalsql-<arch>` | ✓ | ✓ |

Because PG extensions hard-depend on `PG_MODULE_MAGIC`, a binary
compiled for PG 16 will refuse to load under 17 or 18. Install the
package matching your server's major.
...
The bytecode is stripped of debug information (line numbers, local
variable names, chunk names). `fractalsql.so` contains no Lua source
text. This is commercial-grade protection against casual inspection,
not a cryptographic guarantee — a determined reverse-engineer using a
LuaJIT decompiler can recover partial pseudo-Lua with mangled control
flow. If that threat model matters to your deployment, see the
*Intellectual Property* section of the architecture docs for the paths
...
#   postgresql-{16,17,18}-fractalsql-{amd64,arm64}.deb
...
The canonical build is Docker-driven and emits all three `.so`s per
target architecture:

```bash
./build.sh amd64   # -> dist/amd64/fractalsql_pg{16,17,18}.so
./build.sh arm64   # -> dist/arm64/fractalsql_pg{16,17,18}.so
```

The Dockerfile (`docker/Dockerfile`) uses three independent build
stages — `postgres:16-bookworm`, `postgres:17-bookworm`,
`postgres:18-bookworm` — so each `.so`'s `PG_MODULE_MAGIC` matches the
target server's ABI exactly. Cross-arch builds use buildx + QEMU;
CI runs both arches on every tag.
