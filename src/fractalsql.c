...
#include "sfs_core_bc.h"   /* luaJIT_BC_sfs_core[] + luaJIT_BC_sfs_core_SIZE */
...
Datum fractal_search(PG_FUNCTION_ARGS);
Datum fractal_search_debug(PG_FUNCTION_ARGS);
Datum fractal_search_explore(PG_FUNCTION_ARGS);
...
ensure_lua(void)
{
    ...
    rc = luaL_loadbuffer(g_L,
                         (const char *) luaJIT_BC_sfs_core,
                         luaJIT_BC_sfs_core_SIZE,
                         "=sfs_core");
    if (rc != 0) {
